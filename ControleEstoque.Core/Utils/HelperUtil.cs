...
namespace ControleEstoque.Core.Utils
{
    public static class HelperUtil
    {
        ...
        public static void RemoveStringNullValues<T>(ref T entity)
        {
            Type type = typeof(T);
        ...
        public static bool ValidPasswordRules(String password)
        {
            return CheckUpperCase(password) &&
                CheckLowerCase(password) &&
                CheckNumberCharacter(password) &&
                CheckSpecialCharacter(password) &&
                CheckSequencials(password.ToUpper());
        }

        private static bool CheckUpperCase(String test)
        {
            string pattern = @"[A-Z]";
        ...
        private static bool CheckLowerCase(String test)
        {
            string pattern = @"[a-z]";
            Regex regex = new Regex(pattern);
        ...
        private static bool CheckSpecialCharacter(String test)
        {
        ...
        private static bool CheckNumberCharacter(String test)
        {
            string pattern = @"[0-9]";
            Regex regex = new Regex(pattern);
        ...
        private static bool CheckSequencials(String test)
        {
            for (int i = 0; i < test.Length - 1; i++)
            {
                string patternChar = @"[a-zA-Z]";
                Regex regexChar = new Regex(patternChar);

                if (regexChar.IsMatch(test[i].ToString()) && regexChar.IsMatch(test[i + 1].ToString()))
                {
                    if (test[i + 1] == (char)(((int)test[i]) + 1))
                    {
                        return false;
                    }
                }
                else if (CheckNumberCharacter(test[i].ToString()) && CheckNumberCharacter(test[i + 1].ToString()))
                {
                    if ((int)test[i + 1] == ((int)test[i] + 1))
                    {
                        return false;
                    }
                }
        ...
        public static string CreateSaltWithouSpecialCharacters(int size)
        {
            ...
            {
                chars[i] = _Chars[(int)buff[i] % Count];
            }
        ...
        public static string ObterNomeEnum<TEnum>(int valor) where TEnum : struct, Enum
        {
            if (Enum.IsDefined(typeof(TEnum), valor))
            {
                return Enum.GetName(typeof(TEnum), valor);
            }
            else
            {
                throw new ArgumentException($"O valor '{valor}' não é válido para o enum {typeof(TEnum).Name}");
            }
        }
    }
}